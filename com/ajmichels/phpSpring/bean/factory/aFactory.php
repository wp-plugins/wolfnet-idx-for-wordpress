 ...
 *
 */
abstract class com_ajmichels_phpSpring_bean_factory_aFactory
extends com_ajmichels_common_abstractClass
{
	...
	public function setParent ( com_ajmichels_phpSpring_bean_factory_iFactory $parent )
	{
	 ...
	 * @return com_ajmichels_phpSpring_bean_definition
	 * 
											...
											$isLazyInit = false, $initMethod = '' )
	{
		...
		$beanDefinition = new com_ajmichels_phpSpring_bean_definition( $this );
	...
	protected function createPropertyDefinition (	com_ajmichels_phpSpring_bean_definition &$beanDefinition, 
												$name, $type )
	{
		
		/* Create a new instance of a bean definition and set its properties. */
		$property = new com_ajmichels_phpSpring_bean_property( $beanDefinition );
		...
		switch ( $type ) {
			...
			case 'map':
				foreach( $complexProperty as &$entry ) {
					...
					if ( is_object( $entry ) 
						&& $this->getMetaData( $entry )->getName() == 'com_ajmichels_phpSpring_bean_reference' ) {
